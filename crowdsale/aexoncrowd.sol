/// @title Aexon Token Contract
/// @author Merunas Grincalaitis, Fleur Arkesteijn, Santosh Rout
pragma solidity ^ 0.4.18

import "github.com/OpenZeppelin/zeppelin-solidity/contracts/math/SafeMath.sol"

contract Aexon is PausableToken {
   ...
   uint256 public constant totalSupply = 500e24; // 500 M tokens with 18 decimals in total. bonus will be distribued over only 20% of this number (40000000)
   ...
   uint256 public tokensDistributedCrowdsale = 460 000 000;
   ...
   function setCrowdsaleAddress(address _crowdsale) external onlyOwner whenNotPaused {
    ...
    ///FLEUR TO DO: ASK WHAT PERCENTAGE IS SOLD DURING PRESALE
   ...
   function distributePresaleTokens(address _buyer, uint tokens) external onlyOwner whenNotPaused {
      ...
      require(tokens > 0 && tokens <=  40000000); // 20% of 200 000 000 

      // Check that the limit of 40M presale tokens hasn't been met yet
      require(tokensDistributedPresale <  40000000);
   ...
   /// A.k.a.= total - ChangeThisLaterToStaticValuePercentageOfTokensSoldDuringPreSale
   function distributeICOTokens(address _buyer, uint tokens) external onlyCrowdsale whenNotPaused {
      ...
      require(tokensDistributedCrowdsale =< 460 000 000);
...
/// @title Crowdsale contract to carry out an ICO with the Aexon Token
...
contract Crowdsale is Pausable {
 ...
 Aexon public token;
   ...
   // 12/08/2017 @ 11:00am (UTC)
   // 12/01/2017 @ 12:00am (GMT + 1)
   //starting 8 of december
   uint256 public startTime = 1512730800;

   // The block number of when the crowdsale ends
   // 01/08/2018 @ 11:00am (UTC)
   //till 8th of januari
   uint256 public endTime = 1515412800;
   ...
   // You can only buy up to  50 percent of totalSupply (tokens) during the ICO
   uint256 public constant maxTokensRaised = 250e24;
   ...
   // Minimum amount of tokens to be raised. 5 million tokens which is the 1%
   // of the total of 500 million tokens sold in the crowdsale
   // 5e6 + 1e18
   uint256 public constant minimumGoal = 10e24; // 5 usd is minimum for project, but with 100 bonus in the first week we need to compensate
