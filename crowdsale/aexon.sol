...
contract Aexon is PausableToken {
   ...
   uint256 public constant totalSupply = 500e24; // 100M tokens with 18 decimals
   ...
   function distributePresaleTokens(address _buyer, uint tokens) external onlyOwner whenNotPaused {
      uint ChangeThisLaterToStaticValuePercentageOfTokensSoldDuringPreSale=0,0;
      require(_buyer != address(0));
      require(tokens > 0 && tokens <=  ChangeThisLaterToStaticValuePercentageOfTokensSoldDuringPreSale);

      // Check that the limit of 10M presale tokens hasn't been met yet
      require(tokensDistributedPresale <  ChangeThisLaterToStaticValuePercentageOfTokensSoldDuringPreSale);
   ...
   function distributeICOTokens(address _buyer, uint tokens) external onlyCrowdsale whenNotPaused {
      require(_buyer != address(0));
      require(tokens > 0);
      uint ThisTooNeedsToBeDefinedInALaterStageButWillBeTotalMinusTokensSoldDuringPreSale=0,0;
      // Check that the limit of 50M ICO tokens hasn't been met yet
      require(tokensDistributedCrowdsale < hisTooNeedsToBeDefinedInALaterStageButWillBeTotalMinusTokensSoldDuringPreSale);
...
contract Crowdsale is Pausable {
 ...
 Aexon public token;
   ...
   // 7.5e6 + 1e18
   uint256 public constant minimumGoal = 5e24;
